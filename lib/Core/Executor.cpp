...
void Executor::terminateStateOnError(ExecutionState &state,
  ...
  if (interpreterOpts.AbstractFunctions && state.abstraction == ExecutionState::Abstract) {
    ...
    if (lastConc->failCount >= ABSTRACTION_FAIL_THRESHOLD) {
      ...
      // The original ptreenode will be deleted when the abstract state is
      // deleted. Create a new node and detach the old one from the tree.
      PTreeNode *oldNode = lastConc->ptreeNode;
      PTreeNode *parent = oldNode->parent;
      lastConc->ptreeNode = new PTreeNode(oldNode->parent, lastConc);
      if (parent->left == oldNode) {
        parent->left = lastConc->ptreeNode;
      } else {
        assert(parent->right == oldNode);
